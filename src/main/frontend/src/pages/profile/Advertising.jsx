...
const Advertising = () => {
    ...
    const handleClickSend = () => {
        ...
        if(userData === null){
            ...
            alert('문의 내용을 모두 입력해주세요 :)');
        } else {
            const text = '(1) 연락처 혹은 이메일 : ' + contact + ', (2) 문의사항 : ' + description;
            addUserFeedback(userData.user_id, text, '광고 입점 문의');
        }
    };
    ...
    const addUserFeedback = async (user_id, text, type) => {
        ...
        try {
            ...
            alert('감사합니다 :) 빠른 시일 내에 연락드리도록 하겠습니다.');
            navigate('/profile');
        } catch (error) {
            console.log(error);
        }
    ...
    return (
        <Container>
            <HeaderBackButton title="광고 및 협력 문의" backUrl="/profile" />
            <div style={styles.contentDiv}>
                <input
                    name="contact"
                    type="text"
                    placeholder="[필수] 연락처 혹은 이메일을 입력해주세요"
                    style={styles.input}
                />
                <textarea
                    id="description"
            ...
            <ButtonContainer>
                <div style={{ ...styles.buttonDiv }} onClick={() => handleClickSend()} >
                    <div style={{ ...styles.button, ...styles.row }} >
                        <div style={{ ...styles.buttonText }}>문의하기</div>
                    </div>
                </div>
            </ButtonContainer>
        </Container>
